...
def signal_radius(score, mw, relationships):
    ...
    return int(base + score_boost + mw_boost + relationship_boost)
...
with main_tab:
    ...
    if not map_df.empty and {"latitude", "longitude"}.issubset(map_df.columns):
        ...
        if not map_df.empty:
            ...
            map_df["relationship_count"] = map_df["relationship_count"].fillna(0).astype(int)
...
with analytics_tab:
    ...
    with a2:
        ...
        if not filtered_df.empty and "infrastructure_type" in filtered_df.columns:
            infra_chart = (
                filtered_df["infrastructure_type"]
                .fillna("Unknown")
                .value_counts()
                .head(10)
                .reset_index()
